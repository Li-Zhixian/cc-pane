...
- Focused Windows frontend reruns: `powershell.exe -NoProfile -Command "Set-Location 'D:\my-project\cc-pane'; npx vitest run web/ccchan/ChatPanel.test.tsx web/ccchan/installPet.test.ts web/ccchan/deepLink.test.ts --reporter=dot"` verifies ccchan UI startup parameters for Claude local, Codex local, Claude WSL, and Codex WSL roles, WSL role restart behavior, hidden chat lifecycle handling, and protocol compatibility without exposing remote install affordances in settings.
