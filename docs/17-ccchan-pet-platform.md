...
- `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/probe-ccchan-windows.ps1 -VerifyDrag` verifies a real Windows mouse-drag path against the running dev mascot window. On 2026-06-04 after the native window-move persistence fix, it moved the mascot from `(847, 134)` to `(895, 170)`, kept it inside the detected screen, and persisted `windowX = 895.0`, `windowY = 170.0`.
