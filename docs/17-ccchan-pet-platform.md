...
- ccchan frontend runtime checks in `web/ccchan/CCChanApp.test.tsx` verify focused-window status filtering, terminal-status fallback updates, hook event bubbles, fallback restoration after a hook event bubble expires, drag-release persistence, and negative logical coordinate persistence for multi-monitor layouts.
...
- `powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Set-Location 'D:\my-project\cc-pane'; npm run probe:ccchan:windows"` verifies the running dev process, a visible `120x120` topmost mascot window, a visible main window, protocol registration exactly targeting the dev exe, persisted dev ccchan config fields, monitor enumeration, and that the mascot window rectangle is inside one detected screen. The same probe supports `-VerifyDrag`, which performs a short Win32 mouse drag on the mascot, verifies the window moved, verifies `windowX/windowY` persisted, then restores the cursor position.
...
- `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/probe-ccchan-windows.ps1 -VerifyDrag` verifies a real Windows mouse-drag path against the running dev mascot window. On 2026-06-04 it moved the mascot from `(81, 224)` to `(129, 260)`, kept it inside the detected screen, and persisted `windowX = 129.0`, `windowY = 260.0`.
- `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/probe-ccchan-windows.ps1 -VerifyTrayToggle` is available as an explicit, non-default Windows UI Automation probe for the native tray menu. It right-clicks the CC-Panes tray icon, searches the native menu for `Show/Hide cc酱`, and verifies `windowVisible` plus mascot visibility after hide/show. This remains opt-in because Windows tray UI Automation depends on taskbar layout, overflow state, menu exposure, language, and desktop focus.
- On the current Windows host, the opt-in tray probe can locate the CC-Panes tray icon after opening the hidden-icons area, but the native Tauri menu item is not consistently exposed to UI Automation, so the probe fails before clicking `Show/Hide cc酱`. The stable automated coverage remains the core tray decision test plus status-bar/window show-hide paths; the physical native tray menu click is still a manual host interaction check.
...
- Launch the dev or built Tauri app on Windows and verify the transparent ccchan WebView2 window, always-on-top, drag movement, tray/status-bar show-hide, settings `windowVisible`, and multi-monitor positioning. Current automated evidence verifies a visible `120x120` topmost dev ccchan window, persisted visibility/position, protocol targeting, that the mascot is inside one detected monitor on a two-monitor Windows host, the tray toggle's core show/hide decision path, the multi-monitor placement algorithm for primary plus negative-coordinate monitors, a real Win32 mouse-drag move plus `windowX/windowY` persistence, and frontend persistence of negative logical coordinates for cross-monitor layouts. Physical cross-monitor drag/drop feel and native tray menu clicks still require manual host interaction; `-VerifyTrayToggle` can be used opportunistically on hosts where the native tray menu item is exposed through UI Automation.
