...
- Launch the dev or built Tauri app on Windows and verify the transparent ccchan WebView2 window, always-on-top, drag movement, tray/status-bar show-hide, settings `windowVisible`, and multi-monitor positioning. Current automated evidence verifies a visible `120x120` topmost dev ccchan window, persisted visibility/position, protocol targeting, that the mascot is inside one detected monitor on a two-monitor Windows host, and the tray toggle's core show/hide decision path; physical drag/drop movement across monitors and native tray menu clicks still require manual host interaction.
