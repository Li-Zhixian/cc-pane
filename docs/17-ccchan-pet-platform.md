...
- ccchan frontend runtime checks in `web/ccchan/CCChanApp.test.tsx` verify focused-window status filtering, terminal-status fallback updates, hook event bubbles, and fallback restoration after a hook event bubble expires.
