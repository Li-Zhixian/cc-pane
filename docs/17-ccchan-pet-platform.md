...
- `cmd.exe /C "cd /d D:\my-project\cc-pane && npx vitest run web/components/settings/CCChanSettings.test.tsx"` passed on 2026-06-05 with 15 tests, including quick Claude/Codex local/WSL role templates, neutral pet-source labels without remote resource-link affordances, custom local pet-source directory handling, folder install, zip preview/install, install cancellation, and user-pet deletion fallback behavior.
...
- A rerun of `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/probe-ccchan-windows.ps1` on 2026-06-04 after the WSL Claude MCP path fix found the dev process `94604`, a visible topmost ccchan mascot at physical `(-1535, -31)` with size `180x180` on a 150% scaled monitor, `mascotInsideScreen=true`, protocol registration targeting `D:\my-project\cc-pane\target\debug\cc-panes.exe`, and persisted dev config fields `windowVisible = true`, `windowX = -1535.3333333333333`, `windowY = -30.666666666666668`.
- A rerun after backend `WindowEvent::Moved` persistence on 2026-06-04 found dev process `91196`, a visible topmost ccchan mascot at physical `(-1299, 659)` with size `180x180`, `mascotInsideScreen=true`, protocol registration targeting `D:\my-project\cc-pane\target\debug\cc-panes.exe`, and persisted dev config fields `windowVisible = true`, `windowX = -1298.6666666666667`, `windowY = 658.6666666666666`. The matching `-VerifyNativeMove` run moved the same mascot from physical `(-1299, 659)` to `(-1245, 701)` and reported `configMoved=true`, persisting logical `windowX = -830.0`, `windowY = 467.3333333333333` on the 150% scaled monitor.
...
- After the tray probe was hardened to retry stale UI Automation tray-icon elements, recover from a previously hidden mascot, and prefer the Win32 native popup menu item, `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/probe-ccchan-windows.ps1 -VerifyTrayToggle` passed on 2026-06-05 against dev PID `72192`: `prepareMethod=win32-native-menu-item`, `hideOk=true`, `showOk=true`, `hideMethod=win32-native-menu-item`, `showMethod=win32-native-menu-item`, `configAfterHide=false`, and `configAfterShow=true`.
- On the current Windows host, `-DumpTrayMenu` correctly locates the real CC-Panes tray icon as `SystemTray.NormalButton` with name `CC-Panes [DEV]` and now exposes the native popup menu entries through the Win32 menu API. `-VerifyTrayToggle` uses that native menu path instead of blind keyboard selection, so tray hide/show behavior is covered by both the core tray decision tests and a Windows host runtime probe.
...
- A dev REST rerun after the fix launched WSL Claude session `91f711e8-1133-4bef-9a6d-55ff1bb5c6b3` through orchestrator port `58258`, entered `create_session: WSL mode` with `remote_path=/mnt/d/my-project/cc-pane`, spawned `C:\WINDOWS\system32\wsl.exe`, and wrote `C:\Users\ROG\.cc-panes-dev\wsl-claude-mcp-91f711e8-1133-4bef-9a6d-55ff1bb5c6b3.json` pointing at the dev orchestrator. The previous `MCP config file not found: D:\mnt\c\...` failure did not recur; the session still exited with code 1, so the remaining gap is the interactive Claude WSL transcript/auth behavior rather than the MCP config path.
- A later WSL CLI guard rejects WSL roles when the selected CLI resolves to `/mnt/*` directly or through a symlink. On the current host, the expected WSL Claude behavior is now an actionable `CC-Panes WSL error: claude resolves to Windows executable at /mnt/c/...` message instead of a misleading MCP config path failure. A successful interactive WSL Claude transcript still requires installing/signing into the Linux Claude CLI inside the WSL distro; detecting and reporting the Windows shim is the verified product behavior for this host.
...
- Launch the dev or built Tauri app on Windows and verify the transparent ccchan WebView2 window, always-on-top, drag movement, tray/status-bar show-hide, settings `windowVisible`, and multi-monitor positioning. Current automated evidence verifies a visible topmost dev ccchan window at the expected logical pet size, persisted visibility/position, protocol targeting, that the mascot is inside detected monitors, the tray toggle's core show/hide decision path, the Windows native tray menu show/hide path, the multi-monitor placement algorithm for primary plus negative-coordinate monitors, native Win32 movement plus `windowX/windowY` persistence, and frontend persistence of negative logical coordinates for cross-monitor layouts. Physical cross-monitor drag/drop feel still requires manual host interaction because injected mouse-drag probes remain sensitive to focus, cursor injection, auto-wander timing, and DPI state.
